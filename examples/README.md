...
| [`chain-per-building.json`](chain-per-building.json) | The chain repeated per building: two buildings, each with its own `resolvent-city2tabula`, `resolvent-ignis` and `resolvent-buem`, sharing one `resolvent-weather`. A reference binds by name, so every referenced resolvent carries a distinct one (`building_b1`, `typology_b2`); a name used twice is rejected rather than matched per building. Resolvents whose filled parameters are identical are fetched once, which is why the two typology lookups collapse into a single call. It illustrates chaining and the unique-name rule inside a `model.timeseries` registry. It is **not** the payload shape a MEME simulation receives: that carries series already aggregated to one node per transformer area, and no resolvents at all. |
| [`pylovo-generate-grid.json`](pylovo-generate-grid.json) | A proxy target that creates something: the polygon is handed to pylovo's grid generator unresolved, with `max_attempts: 1` and `retry_on_timeout: false` so a grid is never generated twice. The response carries a footprint per building, so the model has to stay small enough for the response to fit `upstream.max_response_bytes`. |
