...
docker run -d --name tentacron -p 8080:8080 \
  ...
  ghcr.io/enerplanet/tentacron:0.4.0-alpha
...
docker run --rm -v /etc/tentacron/config.yaml:/etc/tentacron/config.yaml:ro \
  --env-file /etc/tentacron/secrets.env ghcr.io/enerplanet/tentacron:0.4.0-alpha validate
```
...
release newer than itself. Schema migrations apply on the new binary's
